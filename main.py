...
@app.route("/sendMessage1", methods=['POST','GET'])
def sendMessageTwilio():

    account_sid = 'AC0f4ee5620feeb07d086912183074fd33'
    auth_token = 'a18305692d88452064781e760936a4da'
    number = '+12183166896'

    defaulterStudent = []
    if request.method=='POST':
        criteriaPercentage = request.form.get('criteriPercentage')
        message = request.form.get('message')
        for AttenPer in AttendancePercentage.keys():
            if AttendancePercentage[AttenPer] < int(criteriaPercentage):
                defaulterStudent.append(AttenPer)

        print(defaulterStudent)
        for student in Student_info.query.all():
            if student.PRN in defaulterStudent:
